...
serve(async (req) => {
  ...
  try {
    ...
    if (!inbox_id || !text) {
      throw new Error('inbox_id and text are required');
    }

    console.log(`Parsing message for inbox ${inbox_id}: ${text.substring(0, 100)}...`);
    ...
    // Get automation rules for context
    const { data: rules } = await supabase
      .from('automation_rules')
      .select('pattern, action_json')
      .eq('company_id', company_id)
      .eq('is_active', true)
      .limit(20);
...
CONTEXTO DA EMPRESA:
- Categorias: ${JSON.stringify(categories?.slice(0, 10) || [])}
- Contas: ${JSON.stringify(accounts?.slice(0, 10) || [])}
- Carteiras: ${JSON.stringify(wallets || [])}
- Fornecedores/Clientes: ${JSON.stringify(counterparties?.slice(0, 10) || [])}

REGRAS DE AUTOMAÇÃO EXISTENTES:
${rules?.map(r => `- Padrão "${r.pattern}" → ${JSON.stringify(r.action_json)}`).join('\n') || 'Nenhuma'}
...
Responda APENAS com JSON válido seguindo o schema.`;
    ...
    const { data: aiRequest } = await supabase
      ...
      .insert({
        ...
        inbox_id,
        model: 'google/gemini-2.5-flash',
        input_json: { text, system_prompt: systemPrompt.substring(0, 500) },
    ...
    if (parsedOutput.transaction?.amount > 10000) {
      riskLevel = 'high';
    ...
    const { data: decision, error: decisionError } = await supabase
      ...
      .insert({
        ...
        needs_confirmation: parsedOutput.needs_confirmation || riskLevel !== 'low',
        missing_fields_json: parsedOutput.missing_fields || [],
        risk_level: riskLevel,
        risk_reasons_json: riskReasons,
        status: riskLevel === 'low' && !parsedOutput.needs_confirmation ? 'approved' : 'awaiting_confirmation',
      })
      .select()
      .single();
