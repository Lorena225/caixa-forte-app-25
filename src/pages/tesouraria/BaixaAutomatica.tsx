...
export default function BaixaAutomatica() {
  const { currentCompany, user } = useAuth();
  const { toast } = useToast();
  const [activeTab, setActiveTab] = useState('arquivo');
  const [titleType, setTitleType] = useState<TitleType>('RECEBER');
  ...
  const { data: openTitles = [] } = useOpenTitles({ title_type: titleType });
  const processSettlement = useProcessSettlement();

  // Simular parsing de arquivo CSV
  const handleFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        // Simular leitura e parsing do arquivo
        ...
        const rows: ParsedRow[] = dataLines.map((line, index) => {
          ...
          // Tentar match com títulos em aberto
          ...
          // Match por document_number (external_key)
    ...
    <MainLayout>
      <div className="space-y-6 animate-fade-in">
        <PageHeader
          ...
          description="Processe baixas em lote a partir de arquivos de retorno"
        />

        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
            <TabsTrigger value="arquivo">Por Arquivo</TabsTrigger>
            <TabsTrigger value="bordero" disabled>
              Borderô (em breve)
            </TabsTrigger>
            <TabsTrigger value="compensacao" disabled>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="bg-muted/50 rounded-lg p-4">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Exemplo: NF001;1500,00;2026-01-15;Pagamento cliente
                    </p>
                  </div>
                </CardContent>
              </Card>
          ...
          <DialogContent>
            ...
            <div className="py-4">
              <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
                ...
                <p className="text-sm">
                  Esta ação irá atualizar o saldo e status dos títulos correspondentes.
                </p>
              </div>
            </div>
            <DialogFooter>
